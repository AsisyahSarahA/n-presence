...
<body class="bg-slate-100 p-8 font-sans">
    ...
    <div class="max-w-4xl mx-auto bg-white p-8 border border-slate-300 shadow-md">
        ...
        <div class="text-center mb-6">
            <h3 class="text-base font-bold uppercase text-slate-800 underline tracking-wide">REKAPITULASI PRESENSI BULANAN SISWA</h3>
            <p class="text-xs text-slate-600 mt-1 font-medium">
                Bulan: <strong>{{ \Carbon\Carbon::create()->month($month)->translatedFormat('F') }} {{ $year }}</strong> | Kelas: <strong>{{ $class->name }}</strong>
            </p>
        </div>
        ...
        <table class="w-full text-left text-xs border-collapse border border-slate-400 mb-8">
            <thead>
                <tr class="bg-slate-100 text-slate-800 font-bold uppercase text-[10px] text-center border-b border-slate-400">
                    ...
                    <th class="py-2.5 px-2 border border-slate-400">Hadir</th>
                    <th class="py-2.5 px-2 border border-slate-400">Terlambat</th>
                    <th class="py-2.5 px-2 border border-slate-400">Izin</th>
                    <th class="py-2.5 px-2 border border-slate-400">Sakit</th>
                    <th class="py-2.5 px-2 border border-slate-400">Alpha</th>
                    <th class="py-2.5 px-2 border border-slate-400">% Kehadiran</th>
                </tr>
            </thead>
            <tbody>
                @forelse($monthlyData as $idx => $row)
                    <tr class="border-b border-slate-300">
                        ...
                        <td class="py-2 px-2 border border-slate-300 text-center font-bold text-emerald-800">{{ $row['hadir'] }}</td>
                        <td class="py-2 px-2 border border-slate-300 text-center font-bold text-amber-800">{{ $row['terlambat'] }}</td>
                        <td class="py-2 px-2 border border-slate-300 text-center font-bold text-blue-800">{{ $row['izin'] }}</td>
                        <td class="py-2 px-2 border border-slate-300 text-center font-bold text-orange-800">{{ $row['sakit'] }}</td>
                        <td class="py-2 px-2 border border-slate-300 text-center font-bold text-red-800">{{ $row['alpa'] }}</td>
                        <td class="py-2 px-2 border border-slate-300 text-center font-bold">{{ $row['percentage'] }}%</td>
                    </tr>
                @empty
                    <tr>
