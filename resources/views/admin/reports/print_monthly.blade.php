...
<head>
    ...
    <title>Rekapitulasi Presensi Bulanan - Kelas {{ $class->name }} - {{ $month }}/{{ $year }}</title>
...
<body class="bg-slate-100 p-8 font-sans">
    ...
    <div class="no-print max-w-4xl mx-auto mb-6 bg-white p-4 rounded-2xl border border-slate-200 flex items-center justify-between shadow-sm">
        <div>
            <h1 class="text-sm font-bold text-slate-800">Pratinjau Cetak Rekapitulasi Presensi Bulanan</h1>
            <p class="text-xs text-slate-500">Kelas {{ $class->name }} | Bulan: {{ \Carbon\Carbon::create()->month($month)->translatedFormat('F') }} {{ $year }}</p>
        </div>
        <div class="flex items-center space-x-2">
            <a href="{{ route('admin.reports.daily', ['tab' => 'monthly', 'monthly_class_id' => $class->id, 'month' => $month, 'year' => $year]) }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl text-xs font-bold transition-all border border-slate-200 flex items-center space-x-1">
    ...
    <div class="print-paper-card max-w-4xl mx-auto bg-white p-8 border border-slate-300 shadow-md">
        ...
        <div class="text-center mb-5">
            <h3 class="text-sm font-extrabold uppercase text-slate-900 tracking-wide underline decoration-2 underline-offset-4">REKAPITULASI PRESENSI BULANAN SISWA</h3>
            <p class="text-xs text-slate-600 mt-1.5 font-medium">
                Bulan: <strong class="text-slate-900">{{ \Carbon\Carbon::create()->month($month)->translatedFormat('F') }} {{ $year }}</strong> &nbsp;|&nbsp; Kelas: <strong class="text-slate-900">{{ $class->name }}</strong>
            </p>
        </div>
        ...
        <table class="w-full text-left text-xs border-collapse border border-slate-400 mb-6">
            <thead>
                <tr class="bg-slate-100 text-slate-900 font-bold uppercase text-[10px] text-center border-b border-slate-400">
                    ...
                    <th class="py-2 px-2 border border-slate-400 w-14">Hadir</th>
                    <th class="py-2 px-2 border border-slate-400 w-16">Terlambat</th>
                    <th class="py-2 px-2 border border-slate-400 w-14">Izin</th>
                    <th class="py-2 px-2 border border-slate-400 w-14">Sakit</th>
                    <th class="py-2 px-2 border border-slate-400 w-14">Alpha</th>
                    <th class="py-2 px-2 border border-slate-400 w-20">% Kehadiran</th>
                </tr>
            </thead>
            <tbody>
                @forelse($monthlyData as $idx => $row)
                    <tr class="border-b border-slate-300">
                        ...
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-bold text-emerald-800">{{ $row['hadir'] }}</td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-bold text-amber-800">{{ $row['terlambat'] }}</td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-bold text-blue-800">{{ $row['izin'] }}</td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-bold text-orange-800">{{ $row['sakit'] }}</td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-bold text-red-800">{{ $row['alpa'] }}</td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-bold text-slate-900">{{ $row['percentage'] }}%</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="9" class="py-6 text-center text-slate-400">Tidak ada data rekapitulasi bulanan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
