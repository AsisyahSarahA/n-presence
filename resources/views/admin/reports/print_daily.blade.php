...
<head>
    ...
    <title>Laporan Presensi Harian - Kelas {{ $class->name }} - {{ $date }}</title>
...
<body class="bg-slate-100 p-8 font-sans">
    ...
    <div class="no-print max-w-4xl mx-auto mb-6 bg-white p-4 rounded-2xl border border-slate-200 flex items-center justify-between shadow-sm">
        <div>
            <h1 class="text-sm font-bold text-slate-800">Pratinjau Cetak Laporan Presensi Harian</h1>
            <p class="text-xs text-slate-500">Kelas {{ $class->name }} | Tanggal: {{ \Carbon\Carbon::parse($date)->translatedFormat('d F Y') }}</p>
        </div>
        <div class="flex items-center space-x-2">
            <a href="{{ route('admin.reports.daily', ['date' => $date, 'class_id' => $class->id]) }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl text-xs font-bold transition-all border border-slate-200 flex items-center space-x-1">
    ...
    <div class="print-paper-card max-w-4xl mx-auto bg-white p-8 border border-slate-300 shadow-md">
        ...
        <div class="text-center mb-5">
            <h3 class="text-sm font-extrabold uppercase text-slate-900 tracking-wide underline decoration-2 underline-offset-4">LAPORAN PRESENSI HARIAN SISWA</h3>
            <p class="text-xs text-slate-600 mt-1.5 font-medium">
                Hari/Tanggal: <strong class="text-slate-900">{{ \Carbon\Carbon::parse($date)->translatedFormat('l, d F Y') }}</strong> &nbsp;|&nbsp; Kelas: <strong class="text-slate-900">{{ $class->name }}</strong>
            </p>
        </div>
        ...
        <table class="w-full text-left text-xs border-collapse border border-slate-400 mb-6">
            <thead>
                <tr class="bg-slate-100 text-slate-900 font-bold uppercase text-[10px] text-center border-b border-slate-400">
                    ...
                    <th class="py-2 px-2 border border-slate-400 w-20">Status</th>
                    <th class="py-2 px-2 border border-slate-400 w-20">Jam Masuk</th>
                    <th class="py-2 px-2 border border-slate-400 w-20">Jam Pulang</th>
                    <th class="py-2 px-2 border border-slate-400 w-20">Terlambat</th>
                    <th class="py-2 px-2.5 border border-slate-400 text-left">Keterangan</th>
                </tr>
            </thead>
            <tbody>
                @forelse($attendances as $idx => $att)
                    <tr class="border-b border-slate-300">
                        ...
                        <td class="py-1.5 px-2.5 border border-slate-300 font-mono text-[11px]">{{ $att->student->nisn ?? '-' }}</td>
                        <td class="py-1.5 px-2.5 border border-slate-300 font-bold text-slate-900">{{ $att->student->name ?? '-' }}</td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-bold">
                            {{ $att->effective_status }}
                        </td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-mono text-[11px]">
                            {{ $att->time_in ? \Carbon\Carbon::parse($att->time_in)->format('H:i:s') : '-' }}
                        </td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-mono text-[11px]">
                            {{ $att->time_out ? \Carbon\Carbon::parse($att->time_out)->format('H:i:s') : 'Belum Pulang' }}
                        </td>
                        <td class="py-1.5 px-2 border border-slate-300 text-center font-medium">
                            {{ $att->late_duration_minutes > 0 ? $att->late_duration_minutes . ' min' : '-' }}
                        </td>
                        <td class="py-1.5 px-2.5 border border-slate-300 text-slate-600">
                            {{ $att->notes ?? '-' }}
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="py-6 text-center text-slate-400">Tidak ada data presensi harian.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
