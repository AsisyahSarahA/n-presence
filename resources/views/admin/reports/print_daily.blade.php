...
<body class="bg-slate-100 p-8 font-sans">
    ...
    <div class="max-w-4xl mx-auto bg-white p-8 border border-slate-300 shadow-md">
        ...
        <div class="text-center mb-6">
            <h3 class="text-base font-bold uppercase text-slate-800 underline tracking-wide">LAPORAN PRESENSI HARIAN SISWA</h3>
            <p class="text-xs text-slate-600 mt-1 font-medium">
                Hari/Tanggal: <strong>{{ \Carbon\Carbon::parse($date)->translatedFormat('l, d F Y') }}</strong> | Kelas: <strong>{{ $class->name }}</strong>
            </p>
        </div>
        ...
        <table class="w-full text-left text-xs border-collapse border border-slate-400 mb-8">
            <thead>
                <tr class="bg-slate-100 text-slate-800 font-bold uppercase text-[10px] text-center border-b border-slate-400">
                    ...
                    <th class="py-2.5 px-2 border border-slate-400">Status</th>
                    <th class="py-2.5 px-2 border border-slate-400">Jam Masuk</th>
                    <th class="py-2.5 px-2 border border-slate-400">Keterlambatan</th>
                    <th class="py-2.5 px-3 border border-slate-400 text-left">Keterangan</th>
                </tr>
            </thead>
            <tbody>
                @forelse($attendances as $idx => $att)
                    <tr class="border-b border-slate-300">
                        ...
                        <td class="py-2 px-3 border border-slate-300 font-mono">{{ $att->student->nisn ?? '-' }}</td>
                        <td class="py-2 px-3 border border-slate-300 font-bold">{{ $att->student->name ?? '-' }}</td>
                        <td class="py-2 px-2 border border-slate-300 text-center font-bold">
                            {{ $att->status }}
                        </td>
                        <td class="py-2 px-2 border border-slate-300 text-center font-mono">
                            {{ $att->time_in ? \Carbon\Carbon::parse($att->time_in)->format('H:i:s') : '-' }}
                        </td>
                        <td class="py-2 px-2 border border-slate-300 text-center">
                            {{ $att->late_duration_minutes > 0 ? $att->late_duration_minutes . ' min' : '-' }}
                        </td>
                        <td class="py-2 px-3 border border-slate-300 text-slate-600">
                            {{ $att->notes ?? '-' }}
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="py-6 text-center text-slate-400">Tidak ada data presensi harian.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

        <!-- Signature Block (Kop Tanda Tangan Resmi) -->
        <div class="mt-12 grid grid-cols-2 text-center text-xs text-slate-800">
            ...
            <div>
                <p>Nangtang, {{ \Carbon\Carbon::parse($date)->translatedFormat('d F Y') }}</p>
                <p class="font-bold">Wali Kelas / Petugas Piket</p>
                <div class="h-20"></div>
                <p class="font-bold underline">( ________________________ )</p>
                <p class="text-[10px] text-slate-500">NIP. ........................................</p>
            </div>
