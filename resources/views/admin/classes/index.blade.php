    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between">
            <div>
                ...
                <h4 class="text-2xl font-black text-slate-800 mt-1">{{ $stats['total_classes'] }}</h4>
                <p class="text-[11px] text-slate-500 mt-0.5">Rombongan Belajar</p>
            </div>
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between">
            <div>
                ...
                <h4 class="text-2xl font-black text-slate-800 mt-1">{{ $stats['total_students'] }}</h4>
                <p class="text-[11px] text-slate-500 mt-0.5">Siswa Aktif</p>
            </div>
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between">
            <div>
                ...
                <h4 class="text-2xl font-black text-indigo-900 mt-1">{{ $stats['teacher_assigned'] }} / {{ $stats['total_classes'] }}</h4>
                <p class="text-[11px] text-slate-500 mt-0.5">Penanggung Jawab Kelas</p>
            </div>
        ...
        <form method="GET" action="{{ route('admin.classes.index') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 flex-1">
            <div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama kelas / wali kelas..."
                    class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-primary text-slate-800">
            </div>
            <div>
                <select name="academic_year_id" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-primary text-slate-800">
            ...
            <div class="flex items-center space-x-2">
                ...
                @if(request()->anyFilled(['search', 'academic_year_id']))
                    <a href="{{ route('admin.classes.index') }}" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition-all border border-slate-200">
                        Reset
                    </a>
                @endif
            </div>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-all">
                            ...
                            <td class="py-4 px-6 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    ...
                                    <a href="{{ route('admin.qr-cards.print', $class->id) }}" target="_blank" title="Cetak Kartu QR Kelas" class="p-2 text-slate-500 hover:text-blue-600 rounded-xl hover:bg-blue-50 border border-slate-200 transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 0 1 3.75 9.375v-4.5ZM3.75 14.625c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5a1.125 1.125 0 0 1-1.125-1.125v-4.5ZM13.5 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 0 1 13.5 9.375v-4.5Z" />
                                        </svg>
                                    </a>
                                    <!-- Edit -->
                                    <button onclick="openEditModal({{ json_encode($class) }})" title="Edit Kelas" class="p-2 text-slate-500 hover:text-indigo-600 rounded-xl hover:bg-indigo-50 border border-slate-200 transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                                    ...
                                    <form action="{{ route('admin.classes.destroy', $class->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kelas ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" title="Hapus Kelas" class="p-2 text-slate-500 hover:text-rose-600 rounded-xl hover:bg-rose-50 border border-slate-200 transition-all">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
...
<div id="modal-create" class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-100 overflow-hidden transform transition-all">
        <div class="px-6 py-4 border-b border-slate-150 flex items-center justify-between bg-slate-50/50">
            <h3 class="font-bold text-slate-800 text-sm">Tambah Kelas Baru</h3>
        ...
        <form action="{{ route('admin.classes.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Nama Kelas</label>
                <input type="text" name="name" required placeholder="Contoh: VII-A" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary transition-all text-slate-800 font-medium">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Tahun Ajaran</label>
                <select name="academic_year_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary transition-all text-slate-800 font-medium">
            ...
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Wali Kelas (Opsional)</label>
                <input type="text" name="homeroom_teacher" placeholder="Nama Guru Wali Kelas" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary transition-all text-slate-800 font-medium">
            </div>
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end space-x-2">
                ...
                <button type="submit" class="px-5 py-2 bg-primary hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-sm transition-all">Simpan Kelas</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal-edit" class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-100 overflow-hidden transform transition-all">
        <div class="px-6 py-4 border-b border-slate-150 flex items-center justify-between bg-slate-50/50">
            <h3 class="font-bold text-slate-800 text-sm">Edit Data Kelas</h3>
        ...
        <form id="form-edit" method="POST" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Nama Kelas</label>
                <input type="text" name="name" id="edit-name" required placeholder="Contoh: VII-A" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary transition-all text-slate-800 font-medium">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Tahun Ajaran</label>
                <select name="academic_year_id" id="edit-year-id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary transition-all text-slate-800 font-medium">
                    @foreach($academicYears as $year)
                        <option value="{{ $year->id }}">{{ $year->name }} {{ $year->is_active ? '(Aktif)' : '' }}</option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Wali Kelas</label>
                <input type="text" name="homeroom_teacher" id="edit-teacher" placeholder="Nama Guru Wali Kelas" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary transition-all text-slate-800 font-medium">
            </div>
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end space-x-2">
                ...
                <button type="submit" class="px-5 py-2 bg-primary hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-sm transition-all">Perbarui Kelas</button>
            </div>
        </form>
    </div>
</div>
