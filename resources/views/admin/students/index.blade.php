...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h3 class="text-lg font-semibold text-slate-800">Daftar Siswa</h3>

        <div class="flex items-center space-x-2">
            <button class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center space-x-2 border border-slate-250">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        <form action="{{ route('admin.students.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select name="class_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all text-slate-800">
            ...
            <div class="flex items-end space-x-2">
                <button type="submit" class="bg-secondary hover:bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-sm flex-1">
                    Filter
                </button>
                @if(request()->anyFilled(['search', 'class_id']))
                    <a href="{{ route('admin.students.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl text-sm font-semibold transition-all border border-slate-200">
                        Reset
                    </a>
                @endif
            </div>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all">
                            <td class="py-3 px-6">
                                @if($student->photo_path)
                                    <img src="{{ asset('storage/' . $student->photo_path) }}" alt="Foto Siswa" class="w-10 h-10 rounded-full object-cover border border-slate-200">
                                @else
                                    <div class="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 font-bold border border-slate-200">
                                        {{ substr($student->name, 0, 1) }}
                                    </div>
                                @endif
                            </td>
                            <td class="py-4 px-6 font-mono text-xs text-slate-600">{{ $student->nisn }}</td>
                            <td class="py-4 px-6 font-medium text-slate-900">{{ $student->name }}</td>
                            <td class="py-4 px-6">
                                <span class="px-2 py-0.5 bg-slate-100 text-slate-800 text-xs font-semibold rounded-lg">
                            ...
                            <td class="py-4 px-6 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('admin.students.edit', $student->id) }}" class="p-1.5 text-slate-400 hover:text-secondary rounded-lg hover:bg-slate-100 transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
...
<div id="modal-create" class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-lg w-full shadow-xl border border-slate-100 transform transition-all max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-slate-150 flex items-center justify-between sticky top-0 bg-white">
        ...
        <form action="{{ route('admin.students.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nisn" value="{{ old('nisn') }}" required placeholder="Contoh: 0081234567"
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all text-slate-800 {{ $errors->has('nisn') ? 'border-red-400 bg-red-50' : '' }}">
                @error('nisn')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="Nama Lengkap Siswa"
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all text-slate-800 {{ $errors->has('name') ? 'border-red-400 bg-red-50' : '' }}">
                @error('name')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <select name="class_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all text-slate-800 {{ $errors->has('class_id') ? 'border-red-400 bg-red-50' : '' }}">
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end space-x-2">
                ...
                <button type="submit" class="bg-primary hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm">Simpan Siswa</button>
            </div>
        </form>
    </div>
</div>
