...
<div class="max-w-xl">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-150 flex items-center justify-between">
            <h3 class="font-bold text-slate-800 text-lg">Ubah Data Siswa</h3>
        ...
        <form action="{{ route('admin.students.update', $student->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">NISN</label>
                <input type="text" name="nisn" value="{{ old('nisn', $student->nisn) }}" required placeholder="Contoh: 0081234567"
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $student->name) }}" required placeholder="Nama Lengkap Siswa"
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all text-slate-800">
                @error('name')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="flex items-center space-x-6">
                    <label class="flex items-center cursor-pointer text-sm text-slate-700">
                        <input type="radio" name="gender" value="L" {{ old('gender', $student->gender) == 'L' ? 'checked' : '' }} required class="w-4 h-4 text-primary border-slate-300 focus:ring-primary">
                        <span class="ml-2">Laki-laki</span>
                    </label>
                    <label class="flex items-center cursor-pointer text-sm text-slate-700">
                        <input type="radio" name="gender" value="P" {{ old('gender', $student->gender) == 'P' ? 'checked' : '' }} class="w-4 h-4 text-primary border-slate-300 focus:ring-primary">
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-start">
                @if($student->photo_path)
                    <div class="flex-shrink-0">
                        <label class="block text-xs font-semibold text-slate-400 mb-1">Foto Sekarang</label>
                        <img src="{{ asset('storage/' . $student->photo_path) }}" alt="Foto Siswa" class="w-20 h-20 object-cover rounded-xl border border-slate-200 shadow-sm">
                    </div>
                @endif
                <div class="flex-1 w-full">
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Ganti Foto Profil</label>
